...
def preprocesar_flota_gspread():
    gc = obtener_cliente_gspread_unificado()
    dict_aviones_default = {"THRUS SR2": 4606562, "PIPER PA 36-375": 3985831, "CESSNA O PIPER PA 25": 3036525, "AIR TRACTOR": 4665109, "CESSNA ASA": 3768500, "CESSNA FUMIGARAY": 3065952}
    dict_drones_default = {"DRONE DATAROT": 84428, "DRONE NORTE": 75518, "DRONE AVIL": 71280, "DRONE GENESYS": 71280}
...
def ejecutar(extraer_numero, fmt_sap, procesar_fecha_pesada):
    ...
    if modo_simulacro:
        ...
        with st.container(border=True):
            ...
            if finca_sim != st.session_state.finca_anterior:
                ...
                # --- 🧠 INTELIGENCIA DE CICLOS REAL M3 ---
    ...
    if 'df_config' not in st.session_state or 'df_config_base' not in st.session_state:
        with st.spinner("📡 Descargando bases maestras de Fincas y Configuración..."):
            gc_maestro = obtener_cliente_gspread_unificado()
            if not gc_maestro:
                st.error("❌ Error de Credenciales: No se pudo inicializar el enlace de Google Drive.")
            else:
                try:
    ...
    with st.container(border=True):
        ...
        if not df_t2.empty:
            ...
            st.error("🚨 CRÍTICO: El sistema no detecta la 'TABLA 2' maestra.")
            st.stop()
