...
# =================================================================
# ⚡ MOTORES ACELERADimport streamlit as st
import pandas as pd
import gspread
import requests
import io
...
# 🎯 SOLUCIÓN RADICAL: Dejamos la función SIN ARGUMENTOS. Al no haber parámetros,
# Streamlit no genera procesos de hashing y el error Unhashable queda sepultado para siempre.
@st.cache_data(show_spinner=False)
def preprocesar_flota_gspread():
    """ Extrae y estructura las tarifas de la flota una sola vez en RAM """
    try:
        # La función absorbe las credenciales directamente del ecosistema de la app
        ...
        # Fallback de Seguridad en caso de corte de comunicación satelital
...
# =================================================================
# 👑 RENDERIZADO VISUAL E INYECCIÓN DE CONTROL DE HANGAR
# =================================================================
...
def ejecutar(extraer_numero, fmt_sap, procesar_fecha_pesada):
    ...
    # 🎯 LLAMADA DIRECTA LIMPIA SIN PARÁMETROS: Carga la flota en 0.00 segundos
    ...
    if modo_simulacro:
        ...
        # --- 📡 1. CONEXIÓN A LA BÓVEDA EN SIMULADOR ---
    ...
    # -----------------------------------------------------------------
    # ⚙️ MÓDULO ORIGINAL DE FACTURACIÓN (BLINDADO Y ACELERADO)
    # -----------------------------------------------------------------
    with st.container(border=True):
        ...
        # =================================================================
        # HANGAR DE DESPLIEGUE (Nace 100% Vacío de forma segura)
        # =================================================================
        with st.container(border=True):
        ...
        if not match_ped.empty:
            ...
            # ⚡ LLAMADA AL MOTOR IA MAESTRO EN MEMORIA RAM
            ...
            for item_data in datos_extraidos_sap:
                ...
                if not df_sab.empty:
                    ...
                    if not match_sabana_global.empty:
                        ...
                        match_pista = match_sabana_global[match_sabana_global.iloc[:, idx_almacen].astype(str).str.strip().str.upper().str.contains(str(pista_sel).strip().upper(), na=False)] if idx_almacen != -1 else match_sabana_global[match_sabana_global.astype(str).apply(lambda x: x.str.strip().str.upper().str.contains(str(pista_sel).strip().upper(), na=False)).any(axis=1)]

                        if not match_pista.empty:
                            try:
                                if idx_saldo != -1:
                                    match_pista['Temp_Sort'] = match_pista.iloc[:, idx_saldo].apply(extraer_numero)
                                    match_pista = match_pista.sort_values(by='Temp_Sort', ascending=True) if not match_pista[match_pista['Temp_Sort'] > 0].empty else match_pista.sort_values(by='Temp_Sort', ascending=False)
                            except: pass
        ...
        st.markdown("<br>### 💰 Liquidación Final (Bóveda SAP)"); m1, m2, m3, m4, m5 = st.columns(5)
        ...
        # =================================================================
        # 🛡️ FILTRO CRÍTICO DE SEGURIDAD CONTRA MISIONES EN CERO
        # =================================================================
        if st.button("💾 DETONAR FACTURA Y GUARDAR EN BÓVEDA", type="primary", use_container_width=True):
            if total_ha_cobro_escuadron == 0:
                st.error("🚫 OPERACIÓN RECHAZADA: No ha ingresado ninguna aeronave (Avión o Dron) en el Hangar de Despliegue. Ingrese los datos reales del reporte de vuelo antes de detonar.")
                st.stop()
            ...
            with st.spinner("🚀 Inyectando datos con Precisión de Francotirador..."):
                try:
                    ...
                    st.balloons(); st.success(f"✅ IMPACTO TOTAL CONFIRMADO. Guardado en fila {f_azul}.")
        ...
        st.stop()ORES INDUSTRIALES (Caché de Lógica y Datos)
# =================================================================

# 🎯 SOLUCIÓN DEFINITIVA: Le ordenamos a la caché que ignore el hash del parámetro si es un objeto de secrets de Streamlit
@st.cache_data(show_spinner=False, hash_funcs={dict: lambda _: None, st.runtime.secrets.Secrets: lambda _: None})
def preprocesar_flota_gspread(gcp_secrets_input=None):
    """ Extrae y estructura las tarifas de la flota una sola vez en RAM """
    try:
        if gcp_secrets_input is not None:
            # ⚡ BYPASS DE SEGURIDAD ATÓMICO: Forzamos la conversión a diccionario puro aquí dentro
            credenciales_puras = dict(gcp_secrets_input)
            gc = gspread.service_account_from_dict(credenciales_puras)
        else:
            gc = gspread.service_account(filename='credenciales.json')
...
def ejecutar(extraer_numero, fmt_sap, procesar_fecha_pesada):
    ...
    # 🎯 AQUÍ ESTÁ EL CAMBIO: Extraemos de forma segura el objeto de secrets
    gcp_secrets = st.secrets["gcp_credentials"] if "gcp_credentials" in st.secrets else None
    dict_aviones, dict_drones = preprocesar_flota_gspread(gcp_secrets)
        ...
        if st.button("💾 DETONAR FACTURA Y GUARDAR EN BÓVEDA", type="primary", use_container_width=True):
            ...
            with st.spinner("🚀 Inyectando datos con Precisión de Francotirador..."):
                try:
                    ...
                    # Escritura veloz en cola de Google Sheets
                    col_azul = hoja_maestra.col_values(1)
                    f_azul = next((i+2 for i in range(len(col_azul)-1, -1, -1) if str(col_azul[i]).strip() != ""), 1)
                    if f_azul > hoja_maestra.row_count: hoja_maestra.add_rows(10)
                    ...
                    # Guardado en Memoria Samurai sin duplicación
                    try:
                        datos_memoria = hoja_memoria.get_all_values()
                        set_existentes = {f"{str(r[0]).strip()}|{str(r[9]).strip().upper()}|{str(r[3]).strip().upper()}" for r in datos_memoria[1:] if len(r) >= 10}
