...
# 💥 IA EMPAREJADORA CIRUGÍA MATEMÁTICA (MATA-INTRUSOS)
@st.cache_data(show_spinner=False, ttl=1800)
def emparejar_coctel_ia(sap_dict_pista, dict_recetas, dict_lideres, dict_fertilizantes, coctel_piloto_base):
    ...
    for iter_id, receta in dict_recetas.items():
        ...
        # 1. PUNTOS POR PRODUCTOS DE LA RECETA QUE SÍ ESTÁN EN SAP
        for p_receta, d_esperada in receta.items():
            ...
            if match_receta:
                puntaje += 1000  # Encontró el producto! Excelente.
            else:
                puntaje -= 2000  # Le falta un producto clave (Ej: Banadak). Castigo fatal.

        # 2. PUNTOS NEGATIVOS POR PRODUCTOS EN SAP QUE NO ESTÁN EN LA RECETA
            ...
            if not sap_en_receta:
                ...
                for f_name in dict_fertilizantes.keys():
                    if f_name == k_sap or (len(k_sap) >= 4 and f_name in k_sap) or (len(f_name) >= 4 and k_sap in f_name):
                        ...
                        break
                if not is_fert:
                    puntaje -= 2000 # SAP tiene un intruso (Ej: SAP trae Kurdo, receta es BN6). Castigo fatal.

        # 3. EL PILOTO SOLO DESEMPATA, NO MANDA
