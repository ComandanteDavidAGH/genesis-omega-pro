...
def emparejar_coctel_ia(sap_dict_pista, dict_recetas, dict_lideres, dict_fertilizantes, coctel_piloto_base):
    """ 🤖 MOTOR IA VECTORIZADO: Encuentra el cóctel óptimo """
    ...
    with st.container(border=True):
            ...
            if not match_ped.empty:
                ...
                for _, r_p in match_ped.iterrows():
                    if len(r_p) >= 7 and "459" in str(r_p.iloc[5]):
                        # Mantiene el puntero correcto de las hectáreas
        ...
        if not match_ped.empty:
            ...
            for _, fila_sap in match_ped.iterrows():
                ...
                # ⚡ SOLUCIÓN EXACTA AL CERO (0.000) EN LA TABLA DE QUÍMICOS
                # El sistema ahora busca en los encabezados reales de fila_sap las palabras clave para encontrar la columna correcta
        ...
        if st.button("💾 DETONAR FACTURA Y GUARDAR EN BÓVEDA", type="primary", use_container_width=True):
            ...
            with st.spinner("🚀 Inyectando datos con Precisión de Francotirador..."):
                try:
                    ...
                    # 🛠️ BLINDAJE ATÓMICO EXPLICITO (Keywords unificadas inmunizadas para gspread v6 mediante batch_update)
                    hoja_maestra.batch_update([{'range': f"A{f_azul}", 'values': [row_azul]}], value_input_option='USER_ENTERED')
                    hoja_apoyo.batch_update([{'range': f"A{f_apoyo}", 'values': [fila_apoyo]}], value_input_option='USER_ENTERED')
