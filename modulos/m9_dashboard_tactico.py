...
def cargar_y_preprocesar_boveda_mando(_descargar_matriz_rapida, _procesar_fecha_pesada, _extraer_numero):
    ...
    columnas = [
        "OS", "BLOQUE", "FINCA", "SECTOR", "AREA_BRUTA", "AREA_FUMIG", "COCTEL", "FECHA", "DIA", "SEMANA",
        "H_TOTAL", "GLN_HA", "VOL_TOTAL", "REND_HR", "REND_MIN", "PILOTO", "HK", "MODELO", "COSTO_AVION",
        "COSTO_HA", "DOMINICAL_HA", "COSTO_FINCA", "VALOR_FACTURAR", "PISTA", "INC_2026", "LIMITE",
        "ALERTA", "VAR_PCT", "COSTO_TOTAL", "PAGO_AVION"
    ]
    ...
    cols_numericas = [
        'AREA_FUMIG', 'REND_HR', 'COSTO_HA', 'DOMINICAL_HA',
        'VALOR_FACTURAR', 'LIMITE', 'COSTO_TOTAL', 'COSTO_AVION'
    ]
...
def ejecutar(descargar_matriz_rapida, extraer_numero, procesar_fecha_pesada):
    ...
    col_año, col_periodo = st.columns([1, 2])
    años_disp = ["TODOS"] + sorted(df_dash['AÑO'].unique().tolist(), reverse=True)
    año_sel = col_año.selectbox("📅 AÑO FISCAL", años_disp, index=0)

    # Tres casillas: Tipo / Trimestre / Mes
    with col_periodo:
        row_tipo, row_trim, row_mes = st.columns([1, 1, 1])
        tipo_periodo = row_tipo.selectbox("Tipo de periodo", ["TODOS", "TRIMESTRE", "MES"])
        trimestres = ["Q1 (Ene-Mar)", "Q2 (Abr-Jun)", "Q3 (Jul-Sep)", "Q4 (Oct-Dic)"]
        meses = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"]
        trim_sel = row_trim.selectbox("Trimestre (si aplica)", ["N/A"] + trimestres, index=0)
        mes_sel = row_mes.selectbox("Mes (si aplica)", ["N/A"] + meses, index=0)
    ...
    # Lógica de filtrado por tipo/trimestre/mes (nueva)
    if tipo_periodo != "TODOS":
        if tipo_periodo == "TRIMESTRE" and trim_sel != "N/A":
            trim_map = {
                "Q1 (Ene-Mar)": 1,
                "Q2 (Abr-Jun)": 2,
                "Q3 (Jul-Sep)": 3,
            ...
            df_filtrado = df_filtrado[df_filtrado['TRIMESTRE'] == trim_map[trim_sel]]
        elif tipo_periodo == "MES" and mes_sel != "N/A":
            df_filtrado = df_filtrado[df_filtrado['MES_NOMBRE'] == mes_sel]

    if finca_filtro != "TODAS":
        df_filtrado = df_filtrado[df_filtrado['FINCA'] == finca_filtro]
    if piloto_filtro != "TODOS":
        df_filtrado = df_filtrado[df_filtrado['PILOTO'] == piloto_filtro]
    if hk_filtro != "TODAS":
        df_filtrado = df_filtrado[df_filtrado['HK'] == hk_filtro]
    ...
    if df_filtrado.empty:
        ...
        with g2:
            ...
            def acortar_fecha(txt):
                try:
                    return txt.split('(')[1].replace(')', '') + " '" + txt[2:4]
                except:
                    return txt
