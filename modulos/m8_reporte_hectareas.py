...
# =================================================================
# 💥 HERRAMIENTA INYECTADA DIRECTAMENTE AQUÍ PARA EVITAR EL ERROR DE "NOT DEFINED"
...
def obtener_cliente_gspread_unificado():
    ...
    # 1. Conexión principal en la Nube (Streamlit Cloud)
    if "gcp_service_account" in st.secrets:
        try:
            ...
            logging.error(f"Fallo interno leyendo secrets principales: {e}")
            
    # 2. Conexión secundaria en la Nube
    if "gcp_credentials" in st.secrets:
        try:
            ...
            logging.error(f"Fallo interno leyendo secrets secundarios: {e}")
            
    # 3. Conexión local (Cuando trabajas en tu PC)
    try:
        return gspread.service_account(filename='credenciales.json')
    except Exception as e:
        st.error("🚨 ALERTA ROJA: No se encontró ninguna credencial válida ni en la nube ni en el PC.")
        return None
# =================================================================
...
def cargar_fuentes_maestras_bi(_descargar_matriz_rapida=None):
    ...
    try:
        ...
        st.error(f"🚨 Error descargando Bóveda Maestra: {e}")
    ...
    try:
        ...
        st.error(f"🚨 Error descargando Bóveda Histórica: {e}")
