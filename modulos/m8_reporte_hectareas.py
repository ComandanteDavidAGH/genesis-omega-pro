...
# =================================================================
# 🚁 RADAR DE HECTÁREAS - MAPEO ESTRICTO Y ANTI-COLAPSO
# =================================================================
def ejecutar(supabase_client, descargar_matriz_rapida=None, extraer_numero_ext=None, procesar_fecha_pesada_ext=None, HAS_MATPLOTLIB=True):
    ...
    try:
        with st.spinner("🛰️ Extrayendo bóveda de datos desde la nube..."):
        ...
        # 🎯 MAPEO ESTRICTO: Tal como lo ordenó, sin adivinar.
        # Pasamos TODO a string para matar el veneno NoneType de raíz.
        datos_limpios = []
        for row in raw_data:
            ...
            datos_limpios.append({
                "OS": r.get("Nº ORDEN", ""),
                "PISTA": r.get("PISTA", "").upper(),
                "HK": r.get("HK", "").upper(),
                "MODELO": r.get("MODELO", "").upper(),
                "FECHA": r.get("FECHA", ""),
                "SEMANA": r.get("SEM", ""),
                "HA_NETAS": extraer_numero(r.get("ÁREA FUMIG.(ha)", "0")),
                "H_PROPORCIONAL": extraer_numero(r.get("RENDIMIENTO (horas)", "0"))
            })
        ...
        if df_rep.empty:
            st.warning("⚠️ No se encontraron operaciones válidas. Verifique que la columna 'ÁREA FUMIG.(ha)' y 'FECHA' tengan datos correctos.")
            return
        ...
        st.success("✅ Base de datos mapeada y purificada con éxito.")
        ...
        if vista_seleccionada == "📊 Resumen Gerencial":
            ...
            # 🎯 ARMADURA VISUAL: Pandas nativo sin 'style' para evadir el crash de PyArrow
            ...
            # Tabla plana formateada sin degradados para no morir de memoria
            df_matriz_str = matriz.applymap(fmt_latino)
