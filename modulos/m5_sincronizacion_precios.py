...
def inicializar_cliente_gspread():import streamlit as st
import pandas as pd
import gspread
import re
import math
...
def ejecutar(supabase_client, extraer_numero, fmt_sap, limpiar_texto_vba, val_seguro):
    ...
    with st.container(border=True):
        ...
        # 📊 BOTÓN NUEVO: MATRIZ DE COMPORTAMIENTO SIN MÁRGENES (PRECIO PLENO x DOSIS)
        with c_btn1:
                    ...
                    try:
                        with st.spinner("🕵️‍♂️ Calculando Comportamiento Operativo (Precio Pleno × Dosis)..."):
                            ...
                            for row in respuesta.data:
                                ...
                                val_costo = row.get('PRECIOS', row.get('COSTO', row.get('costo', 0)))
                                txt_costo = str(val_costo).replace('.', '').replace(',', '.').strip()
                                try: precio_final = float(txt_costo)
                                except: precio_final = val_seguro(str(val_costo))
                                if prod: dict_precios[prod] = precio_final
        ...
        # 🚀 BOTÓN ORIGINAL: EJECUTAR INTEGRACIÓN EN LA SÁBANA
        with c_btn2:
